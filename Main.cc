...
void handle_background( int x, int y, SDL_Surface* source, SDL_Surface* destination, float cameraY )
{
  ...
  offset.y = y + int(cameraY + 0.5 - 143);
  SDL_BlitSurface( source, NULL, destination, &offset );
  //nästnästkommande pga smal bakgrund
...
void Tower_Shoot(float StartX, float StartY, float TargetX, float TargetY, float& xVel, float& yVel)
{
  ...
  narl = TargetX - StartX; //närliggande katet
  mots = TargetY - StartY; //motstående katet
...
int main(int argc, char* args[])
{
  ...
  //std::string saved_level = "Saved_Level.txt";
  int NoHurtCounter = 0;
  ...
  int bgX = 0, bgY = 0;
  //load_level(Platforms, "treelevel.txt", BackgroundName);
  //background = IMG_Load(BackgroundName.c_str());
  //BackgroundName får namnet på bildens desination
  ...
  while( quit == false )
	...
	{
	  Mix_HaltMusic();
	  if(Start_Game_Menu(quit, Game, Start_Menu, Load_Level, Select_Level, screen, event, level, player, Start_Menu_Position, Start_Menu_V) == 1)
	    return 1;
	     
	  while(Select_Level == true)
	    {    
	      if(Select_Level_Menu(Select_Level, Load_Level, Start_Menu, quit, Game, player.Game_Over, screen, event, level, Select_Level_Position, Select_Level_V, player) == 1)
		return 1;
	    }
	}
      ...
      while (player.Game_Over == true)
	{
	  ...
	  if( Game_Over_Menu(Load_Level, quit, Game, Start_Menu, player.Paus_Menu, player.Game_Over, screen, event, screen_height, screen_width, Game_Over_Position, Game_Over_V, level, player) == 1)
	    return 1; 
	}
	...
	{
	  Load_Level = false;
	  load_level(Platforms, level, BackgroundName, player);
	  background = IMG_Load( BackgroundName.c_str() );
      ...
      if (Game == true)
	{
	  ...
	  for(int i = 0; i < Platforms.size(); ++i)  //nytt
	    {
	      ...
	      //enemy_tank = dynamic_cast<Enemy_Tank*>(Platforms[i]);
		...
		mp->Elevate();
	      ...
	      if(enemy_tank != 0)
		{
		  if(player.x > enemy_tank->x - enemy_tank->Range && player.x < enemy_tank->x + enemy_tank->Range)
		    {
		      enemy_tank->Animations = 1;
		      enemy_tank->AI_Tank(player.x);
		    }
		  else
		    enemy_tank->Animations = 0;
		}
	      */
		  ...
		  if(player.x > enemy_creeper->x - enemy_creeper->Range && player.x < enemy_creeper->x + enemy_creeper->Range)
		    {
		      ...
		      //enemy_creeper->Animations = 1;
		    }
		  //else
		    //enemy_creeper->Animations = 0;
		}
	      else if(enemy_tower != 0)
		{
		  ...
		  if((enemy_tower->x - enemy_tower->Range) < player.x && 
			...
			{
			  bullet2 = new MovingObject(enemy_tower->x+enemy_tower->Width/2-5, enemy_tower->y+enemy_tower->Height/2, 10, 10, 0, "bild/laser.png");
		      
			  //enemy_tower->Tower_Shoot(player.x, player.y, bullet2->xVel, bullet2->yVel); fixa
			  Tower_Shoot(enemy_tower->x, enemy_tower->y, player.x, player.y, bullet2->xVel, bullet2->yVel);
